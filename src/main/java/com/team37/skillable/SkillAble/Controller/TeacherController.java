    ...
    public class TeacherController {
        ...
        public ResponseEntity<String> setTeacherRole(@RequestBody Map<String, String> request) {
            try {
                ...
                if (!role.equals("TEACHER")) {
                    return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                            .body("Invalid role for teacher endpoint");
                }

                // Transfer user from student table to teacher table
                teacherService.transferToTeacher(email, role);
                return ResponseEntity.ok("Teacher role set successfully");
            } catch (RuntimeException e) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                        .body(e.getMessage());
            }
